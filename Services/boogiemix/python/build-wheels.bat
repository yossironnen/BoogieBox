@echo off
REM Builds pre-compiled madmom wheels for the BoogieMix bootstrapper.
...
REM
REM madmom 0.16.1 is compatible with Python 3.10-3.12 only:
...
for %%V in (3.12 3.11 3.10) do (
    ...
    if not errorlevel 1 (
        ...
        if not errorlevel 1 (
            "%BUILD_VENV%\Scripts\pip.exe" install -q "Cython>=3" "numpy>=1.24" "setuptools" "wheel"
            if not errorlevel 1 (
