...
async fn run_waveform_map_batch_inner(
    ...
    let counts = analyze_waveform_tracks(&db, tracks).await;
...
/// Runs waveform generation + save for an already-resolved list of tracks.
/// Shared by the generic library-sweep batch and the targeted (explicit
/// track-id) variant below, so both stay in lockstep. Returns
/// `(processed, generated, skipped, errors)`.
async fn analyze_waveform_tracks(db: &DbPool, tracks: Vec<TrackStreamRow>) -> (i64, i64, i64, i64) {
...
/// Generates waveforms for exactly the given track ids (whichever of them
/// are still missing one), bypassing the "due"/schedule/batch-size
/// machinery entirely — used by a BoogieMix build's pre-render wait to
/// prioritize its own tracks ahead of the general library sweep
/// (wip/boogiemix-story-timeline-plan.md §4.7). Deliberately does not use
/// the `WAVEFORM_MAP_RUNNING` guard, for the same reason
/// `bpm_analysis::run_bpm_analysis_for_tracks` doesn't: a targeted call for
/// a handful of playlist tracks should never be blocked by an in-progress
/// background sweep.
pub async fn run_waveform_map_for_tracks(
    db: DbPool,
    track_ids: &[String],
) -> Result<WaveformMapRunResult, String> {
    let started_at = timestamp_now();
    ...
    let tracks = tokio::task::spawn_blocking({
        ...
        move || {
            let conn = db.lock().unwrap_or_else(|p| p.into_inner());
            boogiebox_db::playback::list_tracks_missing_waveforms_for_ids(&conn, &ids)
        }
    })
    .await
    .map_err(|e| e.to_string())?
    ...
    let batch_size = tracks.len() as i64;
    let counts = analyze_waveform_tracks(&db, tracks).await;
    Ok(WaveformMapRunResult {
        started: true,
        in_progress: false,
...
mod tests {
    ...
    #[tokio::test]
    async fn run_waveform_map_if_due_skips_while_priority_gate_is_active() {
        let _guard = RUN_LOCK.lock().await;
    ...
    #[tokio::test]
    async fn run_waveform_map_for_tracks_only_touches_the_given_ids() {
        let _guard = RUN_LOCK.lock().await;
        let db = temp_db("targeted");
        let wanted = seed_track_missing_waveform(&db);
        let other = seed_track_missing_waveform(&db);

        let result = run_waveform_map_for_tracks(db.clone(), &[wanted])
            .await
            .unwrap();
        assert_eq!(
            result.processed, 1,
        ...
        assert_eq!(
            result.skipped, 1,
            "its file is missing on disk, so it's skipped, not generated"
        );

        // The un-requested track is left completely alone.
        let conn = db.lock().unwrap();
        let has_waveform: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM track_waveforms WHERE track_id = ?1",
                params![other],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(has_waveform, 0);
    }

    #[tokio::test]
    async fn run_waveform_map_for_tracks_is_a_no_op_for_an_empty_list() {
        let _guard = RUN_LOCK.lock().await;
        let db = temp_db("targeted-empty");
        let result = run_waveform_map_for_tracks(db, &[]).await.unwrap();
        assert_eq!(result.processed, 0);
        assert_eq!(result.generated, 0);
        assert_eq!(result.skipped, 0);
        assert_eq!(result.errors, 0);
